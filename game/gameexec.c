...
//            j--;
    ...
    case CON_STOPSOUND:
        ...
        if (A_CheckSoundPlaying(g_i,*insptr))
            S_StopEnvSound((short)*insptr,g_i);
        insptr++;
        break;
...
//                OSD_Printf("hitag: %d\n",g_sp->hitag);
    ...
    case CON_ADDAMMO:
        insptr++;
        {
            ...
            DukePlayer_t *ps = g_player[g_p].ps;
    ...
    case CON_ADDWEAPON:
        ...
        {
            int32_t weap=*insptr++, amount=*insptr++;
            VM_AddWeapon(weap, amount, g_player[g_p].ps);
        }
        break;
...
//            SetGameArrayID(j,index,value);
    ...
    case CON_IFNOSOUNDS:
        for (j=MAXSOUNDS-1;j>=0;j--)
            if (g_sounds[j].SoundOwner[0].ow == g_i)
                break;
    ...
    default:
        ...
        G_GameExit("An error has occurred in the CON executor.\n\n");
        break;
    }
    return 0;
}
