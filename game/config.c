...
void CONFIG_SetDefaults(void)
{
    ...
    ud.config.ScreenWidth = 800;
    ud.config.ScreenHeight = 600;
    ud.config.ScreenMode = 1;
    ud.config.ScreenBPP = 32;
    ud.config.useprecache = 0;
...
int32 CONFIG_ReadSetup(void)
{
    ...
    else if (SafeFileExists("fusion.cfg") && ud.config.scripthandle < 0)
    {
        Bsprintf(tempbuf,"The configuration file \"%s\" was not found. "
                 "Import configuration data from \"fusion.cfg\"?",setupfilename);

        i=wm_ynbox("Import Configuration Settings",tempbuf);
        if (i) ud.config.scripthandle = SCRIPT_Load("fusion.cfg");
    }
    ...
    if (ud.config.scripthandle >= 0)
    {
        ...
        // packetrate = 100; // min(max(packetrate,20),120);
		// packetrate removed from config for fusion
...

        {
            ...
            if (ud.config.ScreenBPP != 32) ud.config.ScreenBPP = 32;
        }
...
void CONFIG_WriteBinds(void) // save binds and aliases to disk
{
    ...
    {
        symbol_t *symb;
        fprintf(fp,"// this file automatically generated\n");
