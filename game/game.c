        ...
        default:
            ...
            case PACKET_TYPE_VERSION:
                ...
                if (packbuf[2] != (char)atoi(s_buildDate))
                {
                    ...
                    G_GameExit("You cannot play with different versions of fusion!");
                }
            ...
            case PACKET_TYPE_PLAYER_OPTIONS:
				...
				//  if (other==connecthead)
				//  {
				//  	packetrate = packbuf[i++];
				//  	g_movesPerPacket = packbuf[i++];
				//  }
...
{
    char *s = "Usage: fusion [files] [options]\n"
              "Example: fusion -q4 -a -m -tx -map nukeland.map\n\n"
              ...
              "-j[dir]\t\tAdds a directory to fusion's search list\n"
...
static void G_ShowDebugHelp(void)
{
    char *s = "Usage: eduke32 [files] [options]\n"
              ...
              "-fNUM\t\tpackrates and moves/pack in multiplayer (0-9)\n"
    ...
    if (argc > 1)
    {
        ...
        while (i < argc)
        {
            ...
            if (((*c == '/') || (*c == '-')) && (!firstnet))
            {
                ...
                if (!Bstrcasecmp(c+1,"autoload"))
                {
                    initprintf("Autoload enabled\n");
                    g_noAutoLoad = 0;
                    i++;
                    continue;
                }
                ...
                if (!Bstrcasecmp(c+1,"host"))
                {
                        initprintf("HOST\n");
                        g_networkBroadcastMode = 1;
                        g_networkConnectMode = 0;
						numplayers = 2;
                    g_noSetup = TRUE;
                    firstnet = i;
                    netparamcount = argc - i - 1;
...
// #endif
            ...
            if (firstnet > 0)
            {
                if (*c == '-' || *c == '/')
                {
                    ...
                    {
                        g_networkBroadcastMode = 1;
                        g_networkConnectMode = 1;
                        initprintf("Network mode: peer-to-peer\n");
                    }
                    else if (((c[0] == 'n') || (c[0] == 'N')) && (c[1] == '2'))
                    {
                        g_networkBroadcastMode = 1;
                        g_networkConnectMode = 0;
                        initprintf("Network mode: peer-2-peer\n");
                    }
					else if (((c[0] == 'j') || (c[0] == 'h') || (c[0] == 'J') || (c[0] == 'H')) && ((c[1] == 'o')||(c[1] == 'O')))
                    {
                        g_networkBroadcastMode = 1;
                        g_networkConnectMode = 0;
                        initprintf("Network mode: p2p-host/join\n");
                    }
            ...
            if ((*c == '/') || (*c == '-'))
            {
                ...
                case 'F':
                    c++;
                    if (*c == '0') {g_movesPerPacket = 1; packetrate = 120;}
                    else if (*c == '1') {g_movesPerPacket = 1; packetrate = 240;}
                    else if (*c == '2') {g_movesPerPacket = 1; packetrate = 180;}
                    else if (*c == '3') {g_movesPerPacket = 1; packetrate = 100;}
                    else if (*c == '4') {g_movesPerPacket = 1; packetrate = 50;}
                    else if (*c == '5') {g_movesPerPacket = 1; packetrate = 30;}
                    else if (*c == '6') {g_movesPerPacket = 2; packetrate = 100;}
                    else if (*c == '7') {g_movesPerPacket = 2; packetrate = 60;}
                    else if (*c == '8') {g_movesPerPacket = 2; packetrate = 30;}
                    else if (*c == '9') {g_movesPerPacket = 4; packetrate = 30;}
                    break;
                case 'g':
                case 'G':
                    c++;
...
static void G_CompileScripts(void)
{
    ...
    if (g_loadFromGroupOnly)
    {
        if (userconfiles == 0)
        {
            i = kopen4loadfrommod("FUSION.CON",1);
            if (i!=-1)
            {
                Bsprintf(confilename,"FUSION.CON");
                kclose(i);
            }
            else Bsprintf(confilename,"GAME.CON");
        }
...
//    extern char datetimestring[];
    ...
    {
        if (!wm_ynbox("FUSION","Another Build game is currently running. "
...
//    initprintf("build %d\n",(char)atoi(BUILDDATE));
    ...
    if (preinitengine())
    {
        wm_msgbox("Engine Initialization Error: %s", engineerrstr);
        exit(1);
    }
...
//        addsearchpath(mod_dir);
...
#if defined(POLYMOST) && defined(USE_OPENGL)
    {
        ...
        if (stat(tempbuf, &st) >= 0)
        {
            ...
            {
                Bsprintf(tempbuf,"located an obsolete texture cache in the \"%stexcache\" directory.\n\n",dir);
