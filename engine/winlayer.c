...
static int customxdim = 640, customydim = 480, custombpp = 32, customfs = 0;
...
//        initprintf("  - Creating %s device\n", devicedef[devn].name);
        result = IDirectInput7_CreateDeviceEx(lpDI, &guidDevs[devn], &IID_IDirectInputDevice7, (LPVOID *)&dev, NULL);
...
static void cdsenummodes(void)
{
    ...
    for (i=0;i<nmodes;i++)
    {
		...
		if (modes[i].x >= 640 && modes[i].bpp == 32 && (modes[i].y / 3 * 4 == modes[i].x)) 
		{ 
...
void getvalidmodes(void)
{
    ...
    {
        {1280,960},{1024,768},{800,600},{640,480},{0,0}
    };
    int cdepths[2] = { 0, 0 };
...
#if defined(USE_OPENGL) && defined(POLYMOST)
    if (desktopbpp == 32 && !nogl) cdepths[1] = desktopbpp;
    else cdepths[1] = 0;
#endif
