    ...
    if (bind(mysock,(struct sockaddr *)&ip,sizeof(ip)) != SOCKET_ERROR)
    {
        ...
        if (gethostname(hostnam,sizeof(hostnam)) != SOCKET_ERROR)
            if ((lpHostEnt = gethostbyname(hostnam)))
            {
                ...
                printf("MULTI: This machine's IP is %s\n", inet_ntoa(ip.sin_addr));
            }
        return(1);
    }
    return(0);
...
// FUSION 3/3/2020
// host/join (p2p) 2 player:               3 player: todo            # player: <=MAXPLAYERS todo
// 192.168.1.2     game -host 2            game -host 3              game -host #
// 192.168.1.100   game -join 192.168.1.2  game -join 192.168.1.2    game -join 192.168.1.2
// 192.168.1.4                             game -join 192.168.1.2    ...repeat for all joining
...
int initmultiplayersparms(int argc, char **argv)
{
	...
	if (argv)
	{
		printf("MULTI: Beginning to parse net parameters..\n");
		numplayers = 2;
		danetmode = 2;
		if (packetrate < 30)
		{
			packetrate = 120; // 120 matches ticrate
			printf("MULTI: Using default packet settings\n");
			printf("MULTI: packetrate = 120, moves per packet = 1\n");
			printf("MULTI: -f# to use custom packet settings\n");
		}
    }
            ...
            if ((argv[i][1] == 'N') || (argv[i][1] == 'n'))
            {
                ...
                if (argv[i][2] == '0')
                {
                    ...
                    if ((argv[i][3] == ':') && (argv[i][4] >= '0') && (argv[i][4] <= '9'))
                    {
                        ...
                        printf("MULTI: %d-player game\n", numplayers);
                    }
                    printf("MULTI: Master-slave mode\n");
                }
                else if (argv[i][2] == '1')
                {
                    danetmode = 1;
                    myconnectindex = daindex; daindex++;
                    printf("MULTI: /n1 Peer-to-peer mode\n");
                }
                else if (argv[i][2] == '2')
                {
                    danetmode = 2;
                    ...
                    printf("MULTI: /n2 host/join P2P mode\n");
                }
                continue;
            }
            else if ((argv[i][1] == 'P') || (argv[i][1] == 'p')) continue;
        }
		else if ((argv[i][0] >= '0') && (argv[i][0] <= '9') && ((argv[i][1] < '0') || (argv[i][1] > '9')))
		{
			danetmode = 2;
			numplayers = (argv[i][0]-'0');
			printf("MULTI: host mode\n");
			printf("MULTI: %d-player game\n", numplayers);
			continue;
		}
...
//    extern int totalclock;
    ...
    if (myconnectindex == connecthead) //Player 0 only
    {
        ...
        if (!i)
        {
			...
			// <<<-----------------------todo: send other ips to joiners here
			// eg, if numplayers == 3, send player2 ip to player 3 & vice versa, check if they can talk
			// see PRE___ net code in zdoom
    ...
    else
    {
        ...
        if (tims >= lastsendtims[connecthead]+250)
        {
            ...
            if (ic0 == -1)
            {
                ...
                else if ((pakbuf[k] == 0xab) && (myconnectindex != connecthead))
                {
                            ...
                            ((unsigned int)pakbuf[k+2] < (unsigned int)MAXPLAYERS))
                    {
                        ...
                        netready = 1; // todo, sort out ips from host to joiners
...
int getexternaladdress(char *buffer, const char *host, int port)
{
    ...
    if (wsainitialized == 0)
    {
        ...
        {
            initprintf("MULTI: Winsock error in getexternaladdress() (%d)\n",errno);
            return(0);
        }
        wsainitialized = 1;
    }
    ...
    {
        initprintf("MULTI: gethostbyname() error in getexternaladdress() (%d)\n",h_errno);
        return(0);
    }
    ...
    {
        initprintf("MULTI: socket() error in getexternaladdress() (%d)\n",errno);
        return(0);
    }

    if (connect(mysock, (struct sockaddr *)&dest_addr, sizeof(struct sockaddr)) == SOCKET_ERROR)
    {
        initprintf("MULTI: connect() error in getexternaladdress() (%d)\n",errno);
        return(0);
    }
    ...
    {
        initprintf("MULTI: send() error in getexternaladdress() (%d)\n",errno);
        return(0);
    }
