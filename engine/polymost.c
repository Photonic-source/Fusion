...
int glusetexcompr = 0;
int gltexfiltermode = 0; // GL_NEAREST
int glusetexcache = 0;
int glusetexcachecompression = 0;
...
void polymost_glinit()
{
    ...
    cacheindexptr = 0; // Bfopen(tempbuf, "at");
    if (!cacheindexptr)
    {
        glusetexcache = 0;
...
//    LoadCacheOffsets();
    ...
    // Bstrcat(tempbuf,".cache");
    // unlink(tempbuf);
    cacheindexptr = 0; // Bfopen(tempbuf, "wt");
    if (!cacheindexptr)
    {
        glusetexcache = 0;
        // initprintf("Unable to open cache index: %s\n",strerror(errno));
    ...
    cachefilehandle = -1; // Bopen(TEXCACHEFILE,BO_BINARY|BO_CREAT|BO_TRUNC|BO_APPEND|BO_RDWR,BS_IREAD|BS_IWRITE);
...
static int LoadCacheOffsets(void)
{
    ...
    // Bstrcpy(tempbuf,TEXCACHEFILE);
    // Bstrcat(tempbuf,".cache");
    script = 0; // scriptfile_fromfile(tempbuf);
